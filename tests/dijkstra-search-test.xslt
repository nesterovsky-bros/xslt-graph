  ...
  <xsl:function name="t:create-graph" as="map(*)">
    ...
    <xsl:sequence select="
      map
      {
        ...
        'edge-value': function($edge as map(*)) as xs:integer* { $edge?value },
        'edge-vertices': function($edge as map(*)) as xs:integer+ 
         { 
           $edge?from, $edge?to 
         },
        'vertex-edges': function($vertex as xs:integer) as map(*)*
         { 
            $vertices($vertex) 
         }
      }"/>
